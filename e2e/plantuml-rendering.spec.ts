...
test.describe('PlantUML diagram rendering', () => {
  ...
  test('PlantUML blocks render as SVG images, not raw PlantUML code', async ({ page }) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      await expect(img).toHaveAttribute('src', new RegExp('\\/diagrams\\/diagram_\\d+\\.svg$'));
    }
  });

  test('PlantUML SVG images load successfully without errors', async ({ page }) => {
    const plantumlElements = page.locator('.plantuml');
    const count = await plantumlElements.count();
  ...
  test('PlantUML diagrams are wrapped in pan-zoom container', async ({ page }) => {
    const plantumlElements = page.locator('.plantuml');
    const count = await plantumlElements.count();
