...
test.describe("Perguntas de Defesa", () => {
	...
	test("should display header with correct metadata", async ({ page }) => {
		await expect(
			page.getByText("Sistema de Gestão para Ginásio — MSES"),
		).toBeVisible();
	...
	test("should have 3 buttons per question (Resposta, Dicas, Ref)", async ({
		page,
	}) => {
	...
	test("clicking Dicas button triggers action", async ({ page }) => {
		...
		const targetSection = page.locator(
			".tip-section." + target + ", .ref-section." + target,
		);
	...
	test("answer block contains expected content for question 1", async ({
		page,
	}) => {
		const firstQuestion = page.locator(".question").first();
		const respostaBtn = firstQuestion.locator(".q-btn-resposta");
		...
		await expect(targetSection.locator(".answer-block")).toContainText(
			"desativação",
		);
		await expect(targetSection.locator(".answer-block")).toContainText(
			"RNF-11",
	...
	test("ref block has correct styling label", async ({ page }) => {
		...
		const targetSection = page.locator(".ref-section." + target);
		await expect(targetSection).toHaveClass(/open/);
		await expect(targetSection.locator(".block-label")).toContainText(
			"Referência",
		);
	});
	...
	test("last question (55) has answer, dicas, and ref content", async ({
		page,
	}) => {
	...
	test("each category has expected number of questions", async ({ page }) => {
		...
		const rfQuestionCount = await page
			.locator(".question")
			.filter({
