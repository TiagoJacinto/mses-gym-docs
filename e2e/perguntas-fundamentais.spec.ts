...
test.describe('Perguntas Fundamentais', () => {
  ...
  test('should display document header title', async ({ page }) => {
    await expect(page.getByRole('heading', { name: /Perguntas Fundamentais/ })).toBeVisible();
  });
  ...
  test('should display question numbers 1-20 sequentially', async ({ page }) => {
  ...
  test('should display question text for each question', async ({ page }) => {
    ...
    await expect(firstQuestion.locator('.question-text')).toContainText('Qual é o problema operacional');
  });
  ...
  test('clicking Resposta reveals answer block', async ({ page }) => {
    ...
    await expect(firstQuestion.locator('.answer-content')).toContainText('sistema resolve');
  });
  ...
  test('clicking Dicas reveals tips block', async ({ page }) => {
    ...
    await expect(firstQuestion.locator('.dicas-content')).toContainText('Foca na palavra');
  });
  ...
  test('clicking Referencia reveals reference block', async ({ page }) => {
    ...
    await expect(firstQuestion.locator('.referencia-content')).toContainText('Secção 2.1');
  });

  test('accordion behavior - only one block open at a time per question', async ({ page }) => {
    const firstQuestion = page.locator('.question-item').first();
    const respostaBtn = firstQuestion.locator('.btn-resposta');
    const dicasBtn = firstQuestion.locator('.btn-dicas');
  ...
  test('all answer blocks contain non-empty content', async ({ page }) => {
    ...
    const respostaBtn = firstQuestion.locator('.btn-resposta');
    await respostaBtn.click();
    const content = await firstQuestion.locator('.answer-content').textContent();
  ...
  test('question 20 has correct content', async ({ page }) => {
    ...
    await respostaBtn.click();
    await expect(lastQuestion.locator('.answer-content')).toContainText('injecção SQL');
  ...
  test('footer displays project metadata', async ({ page }) => {
    ...
    await expect(page.getByText('Sistema de Gestão para Ginásio')).toBeVisible();
  });
});