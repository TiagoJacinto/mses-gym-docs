...
describe("QuestionList", () => {
	describe("rendering", () => {
		...
		it("has three buttons per question (Resposta, Dicas, Referencia)", () => {
			render(<QuestionList questions={mockQuestions} />);
			const respostaButtons = screen.getAllByRole("button", {
				name: "Resposta",
			});
			const dicasButtons = screen.getAllByRole("button", { name: "Dicas" });
			const referenciaButtons = screen.getAllByRole("button", {
				name: "Referência",
			});
		...
		it("has 20 Resposta buttons for 20 questions", () => {
			...
			expect(screen.getAllByRole("button", { name: "Resposta" })).toHaveLength(
				20,
			);
		});
	});
	...
	describe("accordion behavior", () => {
		it("clicking Resposta reveals answer content", () => {
			render(<QuestionList questions={mockQuestions} />);
			const respostaBtn = screen.getAllByRole("button", {
				name: "Resposta",
			})[0];
		...
		it("clicking same button again closes content", () => {
			...
			// Open
			fireEvent.click(respostaBtn);
			const openAnswerAfterOpen = document.querySelector(
				".answer-content.open",
			);
			...
			// Close
			fireEvent.click(respostaBtn);
			const openAnswerAfterClose = document.querySelector(
				".answer-content.open",
			);
